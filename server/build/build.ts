...
const createCopyFilesPlugin = (options: CopyFilesPluginOptions): BunPlugin => ({
  name: "copy-assets",
  async setup(build) {
    const {
      inputDir = "src",
    ...
    if (!silent) console.log(sty("bgYellowBright", "\nCopying assets...") + "\n")
    ...
    try {
      ...
      if (!silent) console.log(sty("bgGreen", "\nAssets copied successfully!") + "\n")
    } catch ({ message }) {
      console.error(sty("red", `Error copying assets: ${message}`) + "\n")
    }
  },
})
